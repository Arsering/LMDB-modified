...
/** A log formatter that prints the time at the beginning of each log line. */
class PreciseTimedLogFormatter : public LogFormatter {
 protected:
    inline size_t PrintTimeHead(char* buf, size_t buf_size) {
        auto t = std::chrono::system_clock::now();
        time_t tnow = std::chrono::system_clock::to_time_t(t);
        ...
        // size_t s = std::strftime(buf, buf_size, "%Y%m%d%H%M%S", date);
        assert(s < buf_size);
        ...
        size_t elapsed_microseconds =
            std::chrono::duration_cast<std::chrono::microseconds>(t - midnight).count();
        return s + snprintf(buf + s, buf_size - s, ".%06d: ", (int)(elapsed_microseconds % 1000000));
    }
...
class PreciseTimedModuleLogFormatter : public PreciseTimedLogFormatter {
 protected:
    inline size_t PrintModuleName(char* buf, size_t off, const std::string& module) {
        if (module.empty()) return 0;
        buf[off++] = '[';
...
class Logger {
 ...
 public:
    ...
    static inline Logger& GetMine(const std::string& name = std::string());
...
Logger& Logger::GetMine(const std::string& name) {
    static LoggerManager manager;
    return manager.Get(name);
}
